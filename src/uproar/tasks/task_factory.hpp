...
namespace task
{
template <typename> class factory_callable_base;

template <typename Ret, typename... Args> class factory_callable_base<Ret(Args...)> {
    public:
	using fn_t = Ret(Args...);
...
class functor_factory_callable : public factory_callable_base<Ret(Args...)> {
    public:
	functor_factory_callable(Fn fn) : fn_{ fn }
	{
	}
...
template <typename> class factory_callable;

template <typename Ret, typename... Args> class factory_callable<Ret(Args...)> {
	public:
	factory_callable() = default;
	template <typename Fn>
	factory_callable(Fn fn) : fn_{ new functor_factory_callable<Fn, Ret, Args...>(fn) }
	{
	}
...
class UPROAR_API task_factory
	: public factory_base<std::string, base_task,
			      task_factory<TaskObjectDef, TaskObjectDefValue>>,
    ...
    public:
	using parse_t = factory_callable<void(scope_ptr<base_task> &, const TaskObjectDef &,
				 parse_callback<TaskObjectDefValue> &)>;
	...
	auto record_task(const std::string &key)
	{
		...
		{
			return []() { return parse_t([](auto &x, const auto &y, auto &cb) {
					auto v = static_cast<Type *>(x.get());
					auto parser = parse<TaskObjectDef, Type>{};
    ...
    private:
	template <typename Type> void record_impl(const std::string &key)
	{
